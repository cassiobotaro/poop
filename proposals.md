...
## 4. Bug — `sum()` quebra o invariante "métodos POOP retornam tipos POOP"

`poop/types/list.py:88`, `poop/types/tuple.py:84`, `poop/types/set.py:125`, `poop/types/frozen_set.py:60`, `poop/types/interval.py:76`:
...
Para coleção vazia, retorna o `int` nativo `0` em vez de `Int(0)`. `INFECTIONS.md` é explícito: *"Returning a native type is a bug"* (linha 21). Correção:
...
A anotação também deveria ser `Object` em vez de `Any`.

---

## 5. Bug — `MemoryView.at` tem ternário sem efeito

`poop/types/memory_view.py:33-35`:
...
Os dois ramos do ternário são idênticos. Provavelmente foi um esboço de tratamento para multidimensional/non-byte memoryviews que ficou pela metade. Simplificar:

```python
return Int(self._value[index._value])
```

Se houver intenção de suportar `memoryview` com `format != 'B'`, a lógica precisa ser reescrita corretamente.

---

## 6. Bug — `_poop_dict_from_pairs` usa `range` (proibido pelo POOP)

`poop/transformers/dict.py:697-701`:
...
O código do *runtime* é Python puro, então `range`/`for` são aceitáveis aqui — não passa pelos validators. Não é bug funcional, mas **o `for i in range(...)` é exatamente o que o projeto combate filosoficamente** e fica visível em uma leitura casual. Substituir por iteração mais "Pythonic":
...
A mesma observação vale para os helpers `_poop_*_from` que usam `for` — é coerente, são código de runtime, mas vale revisar onde dá para ser mais conciso.

---

## 7. Bug — `Dict.pop` / `Dict.setdefault` retornam tipos não validados
...
## 8. Bug — `__hash__` desabilitado implicitamente em coleções

`List`, `Set`, `Dict`, `ByteArray` definem `__eq__` mas não definem `__hash__`. Conforme [data model do Python](https://docs.python.org/3/reference/datamodel.html#object.__hash__): *"If a class that overrides `__eq__` does not define `__hash__`, its instances will not be usable as items in hashable collections."* Para tipos mutáveis (`List`, `Set`, `Dict`, `ByteArray`) isso é o comportamento desejado e o silêncio do Python está correto. Para `Tuple`, `FrozenSet`, `Bytes`, `Str`, `Int`, `Float`, `Complex` o `__hash__` está corretamente definido.

**Recomendação.** Adicionar explicitamente `__hash__ = None` em `List`, `Set`, `Dict`, `ByteArray` — torna a intenção explícita e documenta que esses tipos são imutavelmente *unhashable*.

---

## 9. Bug — `__eq__` com tipos numéricos heterogêneos sempre retorna `false`
...
## 10. Inconsistência — `do(block)` retorna `None` em vez de `self`
...
## 11. Inconsistência — `BINDINGS` montado depois da classe em `Try` e `With`
...
## 12. Inconsistência — `With` não está em `poop/types/__init__.py`
...
## 13. Filosofia — `Object.is_instance(type_: type)` recebe tipo Python cru
...
## 14. Filosofia — operadores binários `+`, `-`, `*`, `/`, `<<`, `>>`, `&`, `|`, `^` continuam permitidos
...
## 15. Filosofia — `__getitem__` em `List`/`Tuple`/`Str`/`Bytes` está implementado mas é "morto" para POOP
...
## 16. Filosofia — `Str.__repr__ = __str__` esconde aspas em REPL
...
## 17. Filosofia — REPL imprime resultado mas não armazena `_`
...
## 18. Filosofia — `Boolean.while_true(cond_block, body_block)` é uma mensagem para o objeto errado
...
## 19. Filosofia — duplicação `_TrueClass.while_true` vs `_FalseClass.while_true`
...
## 20. Performance — `from poop.types.boolean import false, true` em todo método
...
## 21. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 22. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 23. Funcionalidade nova — `Transcript`
...
## 24. Funcionalidade nova — `Block` como tipo de primeira classe
...
## 25. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 26. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 27. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 28. Funcionalidade nova — `times` em Int
...
## 30. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 31. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 32. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 33. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 34. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Bugs reais a corrigir.** 4 (sum retorna native 0), 5 (MemoryView.at ternário sem efeito), 7 (Dict.pop inconsistente), 8 (`__hash__ = None` explícito).

**Refatorações de alto impacto.** 1 (factory de validators), 2 (base de transformers), 3 (mixin de coleção iterável), 19 (while_* na base de Boolean), 33 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 9 (eq heterogêneo), 13 (`is_instance` com tipo cru), 14 (operadores binários permitidos), 18 (while_true e o receiver irrelevante), 34 (properties).

**Funcionalidades novas com bom retorno por esforço.** 23 (Transcript), 24 (Block), 26 (respond_to), 27 (perform), 28 (Int.times), 32 (no_async).
