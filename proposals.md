...
---
## 1. Funcionalidade nova — `Block` como tipo de primeira classe
...
## 2. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 3. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 4. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 5. Funcionalidade nova — `times` em Int
...
## 6. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 7. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 8. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 9. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 10. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
## 12. Filosofia — `Boolean.while_true(cond_block, body_block)` é uma mensagem para o objeto errado
...
## 13. Filosofia — duplicação `_TrueClass.while_true` vs `_FalseClass.while_true`
...
**Refatorações de alto impacto completadas.** ✅ 0 (while_* na base de Boolean), 9 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 17 (while_true e o receiver irrelevante), 10 (properties).

**Funcionalidades novas com bom retorno por esforço.** , 1 (Block), 3 (respond_to), 4 (perform), 5 (Int.times), 8 (no_async).

**Renomeações.** 11 (`Interval` → `Range`).
