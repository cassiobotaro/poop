...
## 2. Filosofia — `__getitem__` em `List`/`Tuple`/`Str`/`Bytes` está implementado mas é "morto" para POOP
...
## 3. Filosofia — `Str.__repr__ = __str__` esconde aspas em REPL
...
## 4. Filosofia — REPL imprime resultado mas não armazena `_`
...
## 7. Performance — `from poop.types.boolean import false, true` em todo método
...
## 8. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 9. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 10. Funcionalidade nova — `Transcript`
...
## 12. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 13. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 14. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 15. Funcionalidade nova — `times` em Int
...
## 16. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 17. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 18. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 19. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 20. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 9 (while_* na base de Boolean), 19 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 1 (`is_instance` com tipo cru), 2 (operadores binários permitidos), 5 (while_true e o receiver irrelevante), 20 (properties).

**Funcionalidades novas com bom retorno por esforço.** 10 (Transcript), 11 (Block), 13 (respond_to), 14 (perform), 15 (Int.times), 18 (no_async).
