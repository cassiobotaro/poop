...
---
## 1. Filosofia — `Boolean.while_true(cond_block, body_block)` é uma mensagem para o objeto errado
...
## 2. Filosofia — duplicação `_TrueClass.while_true` vs `_FalseClass.while_true`
...
## 3. Performance — `from poop.types.boolean import false, true` em todo método
...
## 4. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 5. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 6. Funcionalidade nova — `Transcript`
...
## 8. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 9. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 10. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 11. Funcionalidade nova — `times` em Int
...
## 12. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 13. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 14. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 15. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 16. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 5 (while_* na base de Boolean), 15 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 1 (while_true e o receiver irrelevante), 16 (properties).

**Funcionalidades novas com bom retorno por esforço.** 6 (Transcript), 7 (Block), 9 (respond_to), 10 (perform), 11 (Int.times), 14 (no_async).

**Renomeações.** 17 (`Interval` → `Range`).
