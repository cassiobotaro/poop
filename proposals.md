...
---
## 1. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 2. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 3. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 4. Funcionalidade nova — `times` em Int
...
## 5. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 6. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 7. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 8. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 9. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 0 (while_* na base de Boolean), 8 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 17 (while_true e o receiver irrelevante), 9 (properties).

**Funcionalidades novas com bom retorno por esforço.** , 13 (Block), 2 (respond_to), 3 (perform), 4 (Int.times), 7 (no_async).
