...
---
## 1. Inconsistência — `BINDINGS` montado depois da classe em `Try` e `With`
...
## 2. Inconsistência — `With` não está em `poop/types/__init__.py`
...
## 3. Filosofia — `Object.is_instance(type_: type)` recebe tipo Python cru
...
## 4. Filosofia — operadores binários `+`, `-`, `*`, `/`, `<<`, `>>`, `&`, `|`, `^` continuam permitidos
...
## 5. Filosofia — `__getitem__` em `List`/`Tuple`/`Str`/`Bytes` está implementado mas é "morto" para POOP
...
## 6. Filosofia — `Str.__repr__ = __str__` esconde aspas em REPL
...
## 7. Filosofia — REPL imprime resultado mas não armazena `_`
...
## 8. Filosofia — `Boolean.while_true(cond_block, body_block)` é uma mensagem para o objeto errado
...
## 9. Filosofia — duplicação `_TrueClass.while_true` vs `_FalseClass.while_true`
...
## 10. Performance — `from poop.types.boolean import false, true` em todo método
...
## 11. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 12. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 13. Funcionalidade nova — `Transcript`
...
## 14. Funcionalidade nova — `Block` como tipo de primeira classe
...
## 15. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 16. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 17. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 18. Funcionalidade nova — `times` em Int
...
## 19. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 20. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 21. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 22. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 23. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 1 (mixin de coleção iterável), 12 (while_* na base de Boolean), 26 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 3 (`is_instance` com tipo cru), 4 (operadores binários permitidos), 8 (while_true e o receiver irrelevante), 24 (properties).

**Funcionalidades novas com bom retorno por esforço.** 14 (Transcript), 15 (Block), 17 (respond_to), 18 (perform), 19 (Int.times), 23 (no_async).
