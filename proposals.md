...
---
## 1. Filosofia — `__getitem__` em `List`/`Tuple`/`Str`/`Bytes` está implementado mas é "morto" para POOP
...
## 2. Filosofia — `Str.__repr__ = __str__` esconde aspas em REPL
...
## 3. Filosofia — REPL imprime resultado mas não armazena `_`
...
## 6. Performance — `from poop.types.boolean import false, true` em todo método
...
## 7. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 8. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 9. Funcionalidade nova — `Transcript`
...
## 10. Funcionalidade nova — `Block` como tipo de primeira classe
...
## 11. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 12. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 13. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 14. Funcionalidade nova — `times` em Int
...
## 15. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 16. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 17. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 18. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 19. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 8 (while_* na base de Boolean), 18 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 1 (`is_instance` com tipo cru), 2 (operadores binários permitidos), 4 (while_true e o receiver irrelevante), 19 (properties).

**Funcionalidades novas com bom retorno por esforço.** 9 (Transcript), 10 (Block), 12 (respond_to), 13 (perform), 14 (Int.times), 17 (no_async).
