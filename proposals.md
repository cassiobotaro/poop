...
---
## 1. Funcionalidade nova — `Transcript`
...
## 2. Funcionalidade nova — `Block` como tipo de primeira classe
...
## 3. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 4. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 5. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 6. Funcionalidade nova — `times` em Int
...
## 7. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 8. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 9. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 10. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 11. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 0 (while_* na base de Boolean), 10 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 17 (while_true e o receiver irrelevante), 11 (properties).

**Funcionalidades novas com bom retorno por esforço.** 1 (Transcript), 2 (Block), 4 (respond_to), 5 (perform), 6 (Int.times), 9 (no_async).

**Renomeações.** 12 (`Interval` → `Range`).
