...
## 4. Bug — `MemoryView.at` tem ternário sem efeito
...
## 5. Bug — `_poop_dict_from_pairs` usa `range` (proibido pelo POOP)
...
## 6. Bug — `Dict.pop` / `Dict.setdefault` retornam tipos não validados
...
## 7. Bug — `__hash__` desabilitado implicitamente em coleções
...
## 8. Bug — `__eq__` com tipos numéricos heterogêneos sempre retorna `false`
...
## 9. Inconsistência — `do(block)` retorna `None` em vez de `self`
...
## 10. Inconsistência — `BINDINGS` montado depois da classe em `Try` e `With`
...
## 11. Inconsistência — `With` não está em `poop/types/__init__.py`
...
## 12. Filosofia — `Object.is_instance(type_: type)` recebe tipo Python cru
...
## 13. Filosofia — operadores binários `+`, `-`, `*`, `/`, `<<`, `>>`, `&`, `|`, `^` continuam permitidos
...
## 14. Filosofia — `__getitem__` em `List`/`Tuple`/`Str`/`Bytes` está implementado mas é "morto" para POOP
...
## 15. Filosofia — `Str.__repr__ = __str__` esconde aspas em REPL
...
## 16. Filosofia — REPL imprime resultado mas não armazena `_`
...
## 17. Filosofia — `Boolean.while_true(cond_block, body_block)` é uma mensagem para o objeto errado
...
## 18. Filosofia — duplicação `_TrueClass.while_true` vs `_FalseClass.while_true`
...
## 19. Performance — `from poop.types.boolean import false, true` em todo método
...
## 20. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 21. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 22. Funcionalidade nova — `Transcript`
...
## 23. Funcionalidade nova — `Block` como tipo de primeira classe
...
## 24. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 25. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 26. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 27. Funcionalidade nova — `times` em Int
...
## 29. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 30. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 31. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 32. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
**Bugs reais a corrigir.** 4 (MemoryView.at ternário sem efeito), 6 (Dict.pop inconsistente), 7 (`__hash__ = None` explícito).

**Refatorações de alto impacto.** 1 (factory de validators), 2 (base de transformers), 3 (mixin de coleção iterável), 18 (while_* na base de Boolean), 32 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 8 (eq heterogêneo), 12 (`is_instance` com tipo cru), 13 (operadores binários permitidos), 17 (while_true e o receiver irrelevante), 33 (properties).

**Funcionalidades novas com bom retorno por esforço.** 22 (Transcript), 23 (Block), 25 (respond_to), 26 (perform), 27 (Int.times), 31 (no_async).
