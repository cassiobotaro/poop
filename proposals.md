...
---
## 1. Filosofia — REPL imprime resultado mas não armazena `_`
...
## 4. Performance — `from poop.types.boolean import false, true` em todo método
...
## 5. Performance / ergonomia — `singletons` para Int(0), Int(1), Str("")
...
## 6. Refatoração — `poop/transformers/dict.py:_poop_dict_from` tem ramos quase idênticos para Tuple/List
...
## 7. Funcionalidade nova — `Transcript`
...
## 9. Funcionalidade nova — `Symbol` (Smalltalk imutável e único)
...
## 10. Funcionalidade nova — `Object.respond_to(message: Str) -> Boolean`
...
## 11. Funcionalidade nova — `Object.perform(message_name, *args)`
...
## 12. Funcionalidade nova — `times` em Int
...
## 13. Funcionalidade nova — `--validators-only` / `--transformers-only` / `--explain` no CLI
...
## 14. Funcionalidade nova — fail-fast vs collect-all em Validator
...
## 15. Funcionalidade nova — `ast.AsyncFunctionDef` está bloqueado mas `async`/`await` não têm validator próprio
...
## 16. Funcionalidade nova — `Number` mixin para `Int` + `Float` + `Complex`
...
## 17. Filosofia — `properties` (`@property`) em tipos POOP contradizem "everything is a message"
...
**Refatorações de alto impacto completadas.** ✅ 6 (while_* na base de Boolean), 16 (Number mixin).

**Decisões filosóficas a documentar em INFECTIONS.md.** 2 (while_true e o receiver irrelevante), 17 (properties).

**Funcionalidades novas com bom retorno por esforço.** 7 (Transcript), 8 (Block), 10 (respond_to), 11 (perform), 12 (Int.times), 15 (no_async).

**Renomeações.** 18 (`Interval` → `Range`).
