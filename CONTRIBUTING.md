...
Releases are tag-driven. The maintainer bumps the version, pushes a
tag, and `.github/workflows/release.yml` creates the GitHub Release
with notes auto-generated from the commits since the previous tag.

To cut a release:

1. Bump `version` in `pyproject.toml` (SemVer — see the conventions
   below).
2. Run `uv lock` so `uv.lock` records the new version.
3. Commit: `git commit -am "chore: release vX.Y.Z"`.
4. Tag and push:
   `git tag -a vX.Y.Z -m "vX.Y.Z" && git push origin main --follow-tags`.
5. Watch the **Release** workflow on GitHub — it verifies the tag is
   on `main`, that `pyproject.toml` matches the tag, and that
   `uv.lock` is in sync, then publishes the GitHub Release.

The release notes are generated by `gh release create --generate-notes`
from commit subjects since the previous tag, so following [Conventional
...
There is no `CHANGELOG.md` in the repo — the GitHub Releases page is
the changelog.
