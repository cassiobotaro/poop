...
- **A bare builtin name is the converter, not the class** (`poop/types/_alias.py`): every converter transformer has two rewrites — `visit_Call` sends `list(x)` to the converter, `visit_Name` sends a bare `list` somewhere else — and for a long time the second went to the wrapper *class*, whose constructor means "build from these elements" rather than "convert". `_arity.py` closed exactly that gap for the direct call; binding the name first reopened it one indirection away, and mostly in silence: `x = list; x([1, 2])` answered `[[1, 2]]`, `x = tuple; x([1, 2])` answered `([1, 2],)`, and `x = int; x(4.9)` answered an `int` **holding 4.9**, which then said `float does not understand #+ with an int` — a value whose class and contents disagree. Not an exotic spelling either: a constructor is an object, so `[list, set].at(0)([1, 2])` and `lambda c: c([1, 2])` are the same program, and passing a converter as a block is ordinary where iteration is `map`/`filter`. The class cannot simply *be* the converter — `poop/types/` builds its values by calling these classes directly (`Str(...)`, `List(*elements)`), which is the other meaning — so each bare name binds a `_poop_<name>_cls` **alias**: a subclass of the wrapper whose metaclass answers a *call* with the converter (guarded on `cls.__dict__`, so `class Stack(list)` still constructs a `Stack`). It stays a class because that is what a bare name is otherwise for — `(5).is_instance(int)`, `class Stack(list)`, `int.name()`. The type-argument case is answered by `unalias`, which `is_instance`/`is_subclass` call on both the instance and class sides; an `__instancecheck__` delegating to the wrapped class (the shape `PoopExcMeta` uses for the mirrors) cannot work here, because the alias *is* a subclass and `ABCMeta`'s subclass walk comes straight back to it until the stack gives out. `slice` needs no alias: it has no converter, and `Slice(start, stop, step)` already is the call.

- **The wording rule is swept mechanically, not program by program** (`tests/test_no_python_wording.py`): the sweep had two halves — a static one walking every `MIRRORS[...]` literal, and a list of failing *programs* whose own docstring called it the weak one ("a leak survives simply by not being on it"). A third half now sends **every public message on every wrapper with wrong-typed arguments** and runs the same patterns over what comes back. It found 47 distinct leaked messages across 114 receiver/message sites the day it was written, in families no single program would have surfaced: `isinstance() arg 2 must be a type` from `x.is_instance(…)` — the banned builtin, spelt as the call that replaces it, on all 15 receivers at once; `slice indices must be integers or None or have an __index__ method` from the `start`/`end` of `find`/`index`/`count`/`startswith` and their `r`-twins, naming subscripting *and* a dunder about a `slice` the program never wrote; `unsupported format string passed to list.__format__` and `format() argument 2 must be str` from the two halves of `Object.format`; and `center() argument 2 must be a byte string of length 1, not int`-shaped reports from a dozen text arguments. The refusals now live in `poop/types/_argument.py` — `a_class`, `a_bound`, `text_like`, `byte_order` — one guard per argument *kind* rather than one per receiver, which is why the same sentence reads the same on `Str`, `Bytes` and `ByteArray`. CPython's wrong-arity shape stays exempt: `_cloak`'s docstring says out loud that it only renames the callee.

- **A method is a block, whichever way it is read, and a block on a class is a method** (`poop/types/block.py`, `poop/types/object.py`): `_as_block` had been wired into `get_attr` only — the spelling almost nobody writes — so `"abc".get_attr("upper").print()` answered `<block>` while `"abc".upper.print()` answered `'function' object has no attribute 'print'`: CPython's word for a message, naming the value `function`, which is `Block`'s own cloak, and then refusing what every block answers. `Object.__getattribute__` wraps a bound method now, in a `_MethodBlock` that keeps CPython's arity wording — `cloak` has already worded that one for a message (`str.upper() takes 1 positional argument…`), and `Block`'s rewording would replace the message's name with the word `block` and drag `inspect.signature`, which evaluates a wrapper's `TYPE_CHECKING`-only annotations, into every failed call. `_`-prefixed names are left native, which `is_message` already calls the boundary of the message surface and which is also what makes the hook terminate. The mirror half is `Block.__get__`: a block installed on a class binds to the receiver, so `C.set_attr("greet", lambda self: "hi")` — the one thing a reader reaches for a sanctioned class-side `set_attr` with — works instead of answering `block expects 1 argument, got 0`. Defining only `__get__` makes it a *non-data* descriptor, which is Python's own split and needs no special case: consulted for a class attribute, skipped for one found in the instance's `__dict__`, so a block held as *state* still reads back as itself.

- **The class side refuses an instance message rather than binding it** (`poop/types/meta.py`): `class_side` exists to remove one failure — "`hash(Foo)` answers 'use `obj.hash()` instead' while `Foo.hash()` answered a binding error" — and removed it for `Object`'s protocol only. The wrapper's own messages stayed, so `str.upper()` answered `str.upper() missing 1 required positional argument: 'self'`, naming `self` (a receiver POOP never spells) and "positional argument" (which the wording sweep bans outright); 64 of the 90 names `str.dir()` listed answered that, 32 of 58 on `list`, 30 of 57 on `dict`. `PoopMeta.__getattribute__` refuses them in the shape `class_` and `mro` already use — `#upper asks an instance; send it to one` — and `__dir__` drops them by the rule it already applied to refusing descriptors, so `:methods str` describes one receiver instead of two. Narrow on purpose: only a plain `FunctionType` found on the class's own MRO, since `staticmethod.__get__` answers the *wrapped* function and would otherwise look identical. `get_attr` / `has_attr` read past the refusal through `_reflected` — asking a class for a method by name is reflection, not writing the message, and the unbound function it answers takes its receiver explicitly as it does in Python.

- **`Try` and `With` check both of their arguments where each was written** (`poop/types/try_.py`, `poop/types/with_.py`): each guarded one and deferred the other. `except_` left the exception *kind* to `isinstance` inside `_execute`, so a quoted class name answered `isinstance() arg 2 must be a type, a tuple of types, or a union` — the banned builtin spelt as the call replacing it — and answered it only when the protected block happened to raise; the same mistake said nothing otherwise, and saying nothing means the handler was never installed. It routes through `_argument.a_class` now, unwraps a POOP `Tuple` so Python's own "catch either" spelling works, and refuses a class that is no exception (`#except_ catches exception classes, and int is not one`), as CPython refuses `except int`. `With.do` left its *body* to the deferred call, which answered `'int' object is not callable` — the sentence `_require_block`'s docstring quotes as the thing being avoided, from the construct it names as the one worth optimizing for — and answered it *after* `enter(cm)`, so the manager was acquired and released for a program that was never going to use it. It is checked first now, before the manager block runs, which is the ordering `_protocol` already argues for one step up.

- **The converters' codec argument goes through `_codec.py` like every other** (`poop/transformers/bytes.py`, `poop/transformers/byte_array.py`): `bytes(text, encoding)` reached into `_value` and called `str.encode` itself, so the one encoding argument in the language the guard never saw was the one in a constructor — `bytes("ab", "rot13")` answered `'rot13' is not a text encoding; use codecs.encode() to handle arbitrary codecs`, the exact sentence `_codec.py`'s docstring quotes as the reason it exists, pointing at a module `no_import` forbids. Three decisions came with it: the encoding was *optional* and invented (`bytes("ab")` was `b'ab'`, utf-8 chosen on the program's behalf), a wrong-*typed* one was *ignored* (`bytes("ab", 5)` fell back to utf-8), and the failure was CPython's (`'ascii' codec can't encode character…` under a class no program can spell). The text branch delegates to `Str.encode` now, which is the guarded surface, and requires the encoding as CPython does. `ByteArray` gained the same two- and three-argument form: it mirrors `Bytes` message for message and this was the one half-pair, with `bytearray("ab")` falling through the `Iterable` branch to answer `'str' object cannot be interpreted as an integer` — a message about integers, for an argument that is text. The *decoding* half already did it right and says so in a comment; this is the encoding half catching up.

- **`range` reads its bounds by index, not by `int()`** (`poop/transformers/range.py`): `Range.__init__` takes `operator.index` deliberately, and the converter in front of it called `int(...)`, which *truncates* where `index` refuses — so `range(3.5)` answered `range(0, 3)` and `range(1, 10, 2.0)` a step of 2, silently running over a different sequence than the program describes, and the one guard in the language that gets this right was unreachable through the only spelling a program can write. The non-numeric cases leaked with it (`int() argument must be a string, a bytes-like object or a real number`, naming a call for something spelt `range`). Reading through `index` closes both: CPython's own `'float' object cannot be interpreted as an integer`, which names the wrapper by the builtin it stands for, and the boolean rung still passes because admitting it is what `index` is for.

- **`Boolean` completes the int side: shifts and `from_bytes`** (`poop/types/boolean.py`): everything that makes `bool` an `int` in CPython is re-supplied by hand here, and two members were missing. `__lshift__`/`__rshift__` and their reflected twins were never written — a shift has no boolean-algebra reading, unlike `&`, `|` and `^` — so `True << True` answered `bool does not understand #<<` while both mixed spellings worked, `Int` covering them from its own side with a comment naming this exact hole. They fold to `Int` whatever the operand is, as CPython does. `from_bytes` was the one half-pair in the message family, with the near-miss hint pointing the reader at `#to_bytes`, the message they had not asked for; it answers a `Boolean` rather than an `Int`, against the family's rule, because CPython builds this one through `cls` (`bool.from_bytes(b"\x05", "big")` is `True`).

- **`MemoryView` answers `index` and `count`** (`poop/types/memory_view.py`): every other sequence in the family answers both, and this was the one receiver that could be told an element is there — `includes` is the substitute `no_in` names — and not asked where. `count` takes one argument, as CPython's does, unlike the text wrappers' bounded twins; `index` routes its failure through `_at.no_element_equal_to`, where CPython said `memoryview.index(x): x not found`, the message written as a call with a placeholder where the value belongs. The rest of what the surface sweep reported (`cast`, `strides`, `suboffsets`, `nbytes`, `release`) stays out: it describes memory layout, the one thing POOP has no vocabulary for.

- **A displayed value reads back as itself** (`poop/types/slice.py`): `Slice` takes its components as they are — `slice` accepts any object and the sequence being sliced resolves each one — and displayed them with `str`, so `slice(None, "a", None)` printed `slice(None, a, None)`, whose stop is the *name* `a`, and `slice(None, "", None)` printed `slice(None, , None)`, which is not source at all. `List`, `Tuple` and `Dict` all build their displayed form from `repr` of each element; `Slice` was the one that did not, and it only ever showed on a non-`Int` component, where the two spellings differ.

- **The report names a class a program can write** (`poop/executor.py`): `_describe` read `type(exc).__name__`, so every unknown selector in the language — the most-printed diagnostic there is — reported `MessageNotUnderstood`, POOP's own class, outside `MIRRORS`: unspellable (`NameError: name 'MessageNotUnderstood' is not defined`) while `except_(AttributeError, …)` caught it and `e.class_().name()` answered `AttributeError`. One failure under two names, the disagreement proposal 16 closed for the `Unicode*` family. It reads `poop_class_of` now, which already answered correctly, and `tests/test_no_python_wording.py` sweeps the `_FAILING` programs for the invariant: the class in an uncaught report is one `except_` can be handed.

- **The reader's own file is read the way CPython reads it** (`poop/cli.py`, `poop/interpreter.py`, `poop/repl.py`, `poop/errors.py`): two ways a perfectly ordinary file was mishandled. A byte-order mark — what editors on Windows write by default — survived `encoding="utf-8"` as a literal `U+FEFF` and the tokenizer refused a file `python3` runs, naming a character invisible in every editor; `utf-8-sig` is what CPython's own loader uses, and the REPL's stdin path spends the mark once on the first line, since `input()` does not strip it. And the caret counted characters, so a tab — one character, eight columns — put it in the indentation of every tab-indented file, further off the deeper the nesting. `_caret_column` already existed for this class of bug (bytes versus characters); the quoted line is `expandtabs()`-ed now and the caret measured over the same expansion, which also removes the terminal's own tab stops from the picture — they start at the gutter, not at the file's column 0.

- **`:explain` covers every message a validator composes, not merely every validator** (`poop/repl.py`): the topic list is derived and a per-validator reachability test already existed, but coverage is not reachability. `no_subscript` grew a second message for a *store*, and the `subscript` snippet stayed `x[0]`, a load — so the reader who wrote `xs[0] = 9`, was told to use `obj.at_put(key, value)` and typed `:explain subscript` was shown the *reading* substitute, the one that proposal exists to stop them getting. The snippet is all four spellings now, and a test reads the literal messages out of `poop/validators/` and requires the snippets to report them all, in the shape `test_doc_counts.py` uses for the README's numbers.
...
**The template surface answers for its own failures.** `"{}".format(...)` is POOP's documented template surface — `no_fstring` names it as the substitute — and `_reject_field_access` guards one half of it, but the other half, what the message answers when the template and the arguments disagree, was CPython's in all five modes: `"{a}".format(b=1)` answered a bare `KeyError: 'a'`, the same repr-with-no-sentence `_at.py` was written to close for a missing dict key; `"{}".format()` named the "positional args tuple", Python's calling convention, for a message whose arguments POOP does not describe that way; `"{:d}".format("a")` named a "format code" for an "object of type", the type-level protocol `_message.py` rewrites everywhere else; and the brace errors said `format string`, Python's name for what POOP calls a template. `Str.format` now words all five — `the template asks for 'a', which no argument named: b`, `the template asks for more than the 1 value it was given`, `a str cannot be formatted with 'd'` — and `_reject_field_access` runs inside the same guard, since the brace errors come from the parser *it* runs; its own refusal passes through untouched, on the `PoopExcMeta` test `reword_if_native` uses for the same reason. The five programs joined `_FAILING`.
...
**One predicate, in one place: `is_message` (`poop/types/_selectors.py`).** Four surfaces ask "may user code send this name?" — `dir()`, `:methods`, the near-miss hint, and the REPL's tab-completion — and three carried their own copy of the rule. The fourth, `_PoopCompleter._attr_matches`, spelt it `not name.startswith("__")`, so pressing Tab on `x._` offered `x._value` (the raw Python `str` behind the wrapper, exactly what `_reject_private` exists to keep out of user code) along with `_abc_impl` and `_eq_group`, POOP internals with no meaning in the language at all. The completer was *teaching* the spelling plain attribute access still reaches. `_name_matches` filtered `_poop_`-prefixed keys only, and now filters every `_` name. A second rule joined it in one place for the same reason: `receiver_label` — a class answers its own name, since `type(cls)` is the metaclass — shared by `explain` and by `:methods`, whose header read `PoopExcMeta understands 30 messages` for `ValueError`. The same fix stopped the completer from reading candidate attributes off the *instance*: `getattr(obj, name)` runs a `property` getter, so pressing Tab could execute program code. It uses `inspect.getattr_static` now, which resolves no descriptor at all — going through `getattr` on the *type* stopped working once the class side began refusing instance messages, since `getattr(Int, "abs", None)` answers the default and every message quietly lost its `(`.
...
Reuses the namespace-shadow `_Visitor` over every name a transformer rewrites in *any* position — the 17 lowercase builtins the type transformers send to mangled `_poop_*` globals (`bool`, `int`, `float`, `complex`, `str`, `bytes`, `bytearray`, `memoryview`, `list`, `tuple`, `dict`, `set`, `frozenset`, `range`, `slice`, `enumerate`, `zip`), both spellings of the root (`object`, `Object`), `Ellipsis`, and the 16 exception mirrors. Rebinding one (assignment, class name, or `def`/`lambda` parameter) would silently retarget the interpreter's internals — `str = "x"` replaces the literal constructor, `def m(self, dict)` makes the body operate on the internal `Dict` class — so the validator rejects it with `'<name>' is a POOP builtin name; it cannot be rebound`. Using the names as constructors (`int("5")`), as bases (`class MyErr(ValueError)`), or as message receivers (`ValueError.raise_(…)`) is unaffected.
...
**That sweep has two halves, because the program list is opt-in.** A leak survives simply by not being on it, which is how several accumulated: `complex("abc")` answered `complex() arg is a malformed string`, `(-1).chr()` answered `chr() arg not in range(0x110000)`, and `d.do(lambda p: d.at_put("b", 2))` answered `dictionary changed size during iteration` — a word POOP does not use for a `dict`, describing a `for` loop the program did not write. Worse, POOP *authored* four of those, in messages raised through `MIRRORS`, the mechanism that exists to make a diagnostic POOP's own — `complex`'s three type refusals and `int`'s base refusal, the last four lines below a docstring stating the rule it broke. The runtime half must stay a program list, but the POOP-authored half is statically checkable: the sweep walks `poop/types` and `poop/transformers` for every string literal handed to a `MIRRORS[...]` call and runs the same patterns over it, so a new `complex()` fails the day it is written. One exemption list carries the phrases a message may legitimately quote — a hint showing the POOP spelling (`obj.get_attr(...)`) looks like a Python call to the patterns — in the shape `_EXEMPT` in `test_mirrored_raises.py` already uses. The mutation-during-iteration refusal is caught where POOP iterates (`_IterableMixin.do` and the cursor in `_PeekMixin`), and only a *native* `RuntimeError` is reworded: the test is on `type(exc)`, since `PoopExcMeta` makes a mirror match its native twin on purpose. All three name the collection: `_mutated.iterating` reads the receiver's own cloaked name in `do`, and `_PeekMixin._iterating` carries it for the cursor, derived by `_IteratorBase.__init_subclass__` from the CPython iterator name each concrete iterator already declares (`list_iterator` → `list`, `dict_keyiterator` → `dict`) so a new iterator cannot ship without one. The cursor used to pass the literal `"the collection"`, so `d.do(...)` and `d.iter().next()` reported one fact in two vocabularies — and the anonymous half was the protocol `_peek.py` holds up as idiomatic. The lazy views (`Map`, `Filter`, `Zip`, `Enumerate`) keep the default label, which is honest rather than wrong: the mutated collection is the one behind them, and they cannot name it.
...
POOP mirrors the exceptions it can actually reach: `Exception`, `ArithmeticError`, `LookupError`, `ZeroDivisionError`, `OverflowError`, `IndexError`, `KeyError`, `AttributeError`, `NameError`, `TypeError`, `ValueError`, `RuntimeError`, `NotImplementedError`, `RecursionError`, `AssertionError`, `StopIteration`, `EOFError`. `Try.except_(ValueError, h)` and `ValueError.raise_("msg")` now take a POOP class, closing the last raw primitive in POOP's substitutes for `try` and `raise`.

Seventeen, not "~100+": Python 3.14 has 71 builtin exceptions, and a language with no files and no modules can never raise the `OSError` subtree. `RecursionError` is in the list because recursion is POOP's substitute for every loop, which makes it the most reachable of the lot. The budget it runs against is POOP's too, and is set in `poop/executor.py`: one POOP message send is **six** Python frames — `Block.__call__`, `_MethodBlock.__call__` twice, the branch method, and the reader's own method and block — so under CPython's default limit of 1000 a self-sending message ran 164 levels deep where the same recursion written as a Python function ran 998. The limit is now sized from that frame cost (`1000 * _FRAMES_PER_SEND`), which buys a POOP program roughly the thousand levels a Python program already gets. Two of the six frames are `_MethodBlock.__call__`, the price of "a method read off an object is a block": bypassing that wrap and re-measuring the same program gives 247 instead of 164, so the feature costs a third of the ceiling — worth having written down next to it. Raising the limit is safe rather than a gamble because CPython 3.14 guards the C stack separately and answers a catchable `RecursionError` (`Stack overflow (used 8148 kB) while calling a Python object`) instead of crashing; `tests/test_recursion.py` asserts both the depth and that floor. `while_true` and `while_false` are real Python `while` loops and have no ceiling at all, so this concerns *structural* recursion — a tree walk, where the depth follows the data.

**The reachability argument was checked against the surface, and was one short.** `Str.input` is the one message that reads from outside the program, and its one failure is end of input — reached by nothing more exotic than a pipe, which is what `examples/basics/greet.py` answered `EOFError: EOF when reading a line` to. Without a mirror the class could not be named (`except_(EOFError, …)` → `name 'EOFError' is not defined`), and `except_(Exception, …)` reported the kind as `Exception`: the only I/O POOP has was the only failure it could not handle by name. `input` now raises the mirror with a sentence of its own (`there is no more input to read`).

The `Unicode*` family is the other half of that argument and is answered differently: `encode`/`decode` *can* fail on the text they are given, but POOP does not mirror those classes — `_codec.py` rewords both failures as a `ValueError`, which is what `UnicodeError` is in CPython's tree anyway. Mirroring them would have meant reproducing a five-argument constructor whose `__str__` composes the very `codec` sentence that module exists to keep out, and `poop_class_of` already answers the nearest mirrored ancestor, so a handler was told `ValueError` while the uncaught report printed `UnicodeEncodeError` — one failure under two names. Now both say `ascii cannot encode 'é' at position 1` / `utf-8 cannot decode byte 0xff at position 0`, under one class a program can spell.
...
**The class side is listed by `dir()`.** CPython's `type.__dir__` walks the class's own MRO only, so the metaclass's messages appeared in no discovery surface: `Foo.dir()` answered no `name` and no `superclass`, and `ValueError.dir()` no `raise_` — a documented protocol reachable only by already knowing it exists. (The rest of the class side was listed all along, but for the instance-side reason: `Object` happens to spell `print`, `hash`, `get_attr` the same.) `PoopMeta.__dir__` merges the `class_side` descriptors found on `type(cls).__mro__`, nearest metaclass first — so `raise_` is listed for a mirror, whose `PoopExcMeta` answers it, and not for a plain class, whose `PoopMeta` refuses it. Answered in `__dir__` rather than in the `dir` message, because `dir` is not the only reader: `:methods`, the REPL completer and the near-miss hint all call Python's `dir` and would each have needed their own copy of the merge. A **refusing** descriptor is left out — offering a name that answers "that is Python's, use `#superclass`" teaches worse than not offering it — which is what `class_side_refusal` marks (a flag on the descriptor, not a list of names to keep in step by hand). The same rule now covers the instance-only half, which the merge used to leave in: the list described *two* receivers under a header claiming one, so `:methods str` said `str understands 90 messages` and 64 of them answered a binding error. `class_` and `class_name` go with them — `Object` answers both, but the class refuses them, and this is a list of what *this* receiver understands. Only the names the class really answers survive: `str.dir()` is 24 messages, and every one of them can be read off the class without a refusal. The result is deduped, because the builtin `dir` sorts what `__dir__` hands it but does not dedupe — `type.__dir__` does that internally, and this merge reaches past it. The union is also what puts the shared names back: `print` is a plain method on `Object` *and* a `class_side` message, so it is dropped by the filter and restored by the merge, which is exactly the two-receiver distinction the filter draws.
...
- Every class-side message is a **data descriptor** (`class_side`), not a plain method. Attribute lookup on a class searches the class's own MRO *before* the metaclass, so a plain `PoopMeta.print` would never be reached — `Object.print` would win and answer the unbound function this exists to remove. Only a data descriptor is consulted first. Instances are unaffected: instance lookup never consults the metaclass, so `Foo().print()` still finds `Object.print`. The descriptor cloaks its function when `__set_name__` names it, which is the one moment the function knows the message it answers: `cloak` renames what is in `vars(cls)` and unwraps `classmethod`/`staticmethod` through `__func__`, but a `class_side` keeps its function in `_fn` and `PoopMeta` is never cloaked at all — so `Foo.name(5)` used to answer `PoopMeta.name() takes 1 positional argument but 2 were given`, naming a class a program cannot write, cannot reach and will not find in any documentation aimed at it. It answers `name() takes 1 positional argument…` now: no receiver, but no invented one either. Answering `Foo.print()` is not available here — one function is shared by every class, and `__qualname__` is fixed when the class body runs. Being a data descriptor means it also owns assignment, and it refuses: `Foo.name = 5` and `Foo.set_attr("name", 5)` both answer `#name is answered by every class — it cannot be rebound`. It used to raise `AttributeError(self._name)` — `AttributeError: name`, which reads as if the *word* `name` were the problem and says neither what was refused nor why.
...
`Dict.do` *is* the mixin's — it walks the keys, like every other iteration message on the same receiver. `Bytes` and `ByteArray` override `find` for substring search (different semantics from the mixin's element-finding `find`).
...
- `Dict`'s mixin messages iterate what CPython iterates — the **keys** — so `d.map(block)` matches `map(f, d)`, and `d.items().map(...)` is the pair-shaped spelling. `Dict`'s own `enumerate`/`zip` copies were dropped in favour of the mixin's identical ones.
- **`do` walks the keys too, and used not to.** It was the one message here that answered `Tuple(key, value)` pairs, while `map`, `filter`, `sorted`, `min`, `max` and `iter` on the same receiver all answered keys — one receiver with two answers to "what is an element of a `dict`", and the odd one out was the message `no_loops` names as the substitute for `for k in d`, which walks the keys in Python. So `d.do(lambda k: k.upper())` answered `tuple does not understand #upper` while `d.map(lambda k: k.upper())` worked. The override is gone; `d.items().do(...)` is the pair spelling, and answers `Tuple`s already. The mutation guard came along with it — the mixin's `do` carries the same one, worded from the receiver.
...
**Asking a set about a set is not storing one.** `Set` is unhashable, so `s.includes({1})`, `s.discard({1})` and `s.remove({1})` all answered `cannot use 'set' as a set element (unhashable type: 'set')` — a refusal about *storing*, given to three messages that only look. CPython draws the line the other way: `set.__contains__`, `discard` and `remove` probe with a temporary `frozenset`, so `{1} in s` is an ordinary question (`False`), `s.discard({1})` an ordinary no-op, and `s.remove({1})` reaches its `KeyError` — while `s.add({1})` stays refused. `_set_algebra.probed` makes the same substitution for the four reading messages on `Set` and the two on `FrozenSet`: a mutable `set` operand is answered by an equivalent `FrozenSet`, which hashes and compares exactly as a stored one does, so a set really held inside a set is found rather than blanket-refused. Recognised through the `_set_like` marker `_other_set` already uses, so no `Set` <-> `FrozenSet` import cycle.
...
**The leak is the protocol, not the values passed through it.** `__exit__` used to receive `type(e)`, `e` and `e.__traceback__` — the native class rather than the mirror `except_` matches (and one no program can name), the bare exception rather than the `Error` a `Try` handler is given, and a traceback object: a whole introspection surface, reached through a dunder `no_dunder_attribute` will not let a program spell. On the success path all three were Python's `None`, so `kind.is_none()` — the first thing an `__exit__` written in POOP asks — failed with `'NoneType' object has no attribute 'is_none'` there too. It now receives exactly what `Try` hands a handler: `poop_class_of(e)`, `Error(e)`, and `none` for the traceback POOP does not have and will not invent. The three-argument shape is unchanged, so a manager written to Python's signature still binds.
...
**The superclass ladder steps over the alias.** An alias's one base is the wrapper it stands for, and `cloak` gives both the builtin's name — so climbing from a bare builtin name met a rung that is not there: `int.superclass()` answered a class calling itself `int`, `int.superclass().is_identical(int)` denied it, and `object` was two steps up instead of one. `(5).class_().superclass()` answered `object` correctly all along, so the phantom appeared only under the name a program writes. `PoopMeta.superclass` skips a base that `unalias` maps to the receiver — the same "the pair is one class" decision `__eq__` makes below. A program's own `class Stack(list)` is unaffected: its superclass *is* `list`, the name it wrote.

**Comparing two classes answers a `Boolean`, and the wrapper equals the name that spells it.** `Object.__eq__` answers a POOP object, but a *class* is compared by its metaclass, and `PoopMeta` defined no `==` — so `int == int` fell through to `type.__eq__` and handed a raw Python `bool` to user code, which then answered `'bool' object has no attribute 'print'`: CPython's word for the thing POOP calls a message, from the shortest program that compares two classes. `PoopMeta.__eq__` / `__ne__` answer a `Boolean` and read both sides through `unalias`, so the two spellings of one class are one class — `(5).class_() == int` was `False` before, silently, about two objects that both call themselves `int` (`class_()` answers the wrapper, a bare `int` the alias built on it). `is_identical` deliberately does *not* unalias: it asks identity, and those really are two objects. `__hash__` is re-declared as `type.__hash__`, since defining `__eq__` would otherwise make every POOP class unhashable — `NATIVE_TO_POOP` keys on them.

**A user class can satisfy the protocol slots it is allowed to define.** `class P(Object)` may define dunders — `__init__`, `__eq__`, `__lt__`, `__enter__` and `__add__` all work, and this file tells a contributor to wire dunders to public Python-named methods — but a POOP method can only return POOP values, and CPython reads five of those slots itself and demands a native. Every one was therefore unsatisfiable from inside the language, with a sentence that contradicted itself, because `_cloak` renamed the wrapper to the builtin it stands for: `__str__ returned non-string (type str)`, `__bool__ should return bool, returned bool`, and a `__hash__` refusal that printed `__poop__.P` — the module marker `_reject_builtin` uses to tell a user's class from a builtin, which no program should ever see. `PoopMeta.__new__` adapts `__str__`, `__repr__`, `__bool__`, `__hash__` and `__len__` where the class is built, unwrapping the answer through `_bridge.to_python`; a native answer short-circuits, which is what keeps the wrappers in `poop/types/` untouched and the module importable (`bool(x)` runs during the package's own import). A wrong answer is refused by the *role* rather than the slot — `P's text must be a str, got an int` — since a message spelling `__str__` names the construct `no_dunder_attribute` bans. `__len__` was the quiet member of the family: defining it raised nothing and did nothing, because `len` is how POOP asks and nothing supplied it, so a class declaring `__len__` and no `len` now answers the message too.

**A class-side constructor builds through the wrapper, not through `cls`.** `fromhex` and `from_bytes` are classmethods, so `cls` is whatever the program named — and a bare builtin name is the alias, whose call is the *converter*. A converter takes what a program writes; a classmethod holds a finished Python value, so the three broke in the only spelling a reader would use (`bytes.fromhex("6162")` answered `'int' object has no attribute '_value'`, printing the internal slot name the faithful-unwrap idiom exists to hide; `int.from_bytes(…)` and `float.fromhex(…)` answered `cannot convert int to int` / `cannot convert float to float`, sentences with nothing in them), while the same message sent to an *instance* worked. `_alias.wrapped_instance` answers all three receivers: the alias answers the wrapper's own instance, a plain wrapper builds itself, and a program's `class B(bytes)` is built from the wrapper's constructor so `B.fromhex(…)` still answers a `B`. `ByteArray.fromhex` was added in the same breath — `Bytes` and `ByteArray` mirror each other message for message, and this was the one half-pair, for a spelling CPython supports.

**A subclass of a builtin constructs the way the builtin does.** `_AliasMeta.__call__` read `_converter` off `cls.__dict__` and stopped there, which left the whole convert-versus-build gap open one level down — for the exact spelling `_alias.py` names as a legal use of a bare name (`class Stack(list)`). All thirteen constructors disagreed with themselves there: seven silently (`Stack([1, 2])` answered a `Stack` holding one list where `list([1, 2])` holds two elements; `class N(int)` then `N(4.9)` answered an int holding 4.9, item 9's own example) and six by refusing in Python's words — `dict.__init__() takes 1 positional argument but 2 were given`, `__str__ returned non-string (type int)`, `cannot use 'list' as a set element`. The converter is now looked up along the MRO and its answer rebuilt as `cls`, slot by slot: a subclass has the wrapper's slots, being a subclass, and `_payload_slots` reads them off the `__slots__` declarations rather than a table, so a new wrapper is covered by having one. The payload is copied, since a converter may answer a value it was handed. Two things keep their old meaning: a subclass declaring its own `__init__` is built by it (what the `__dict__` read was protecting), and `bool` — whose two values are singletons with no payload — passes its converter's answer through.

**A subclass is born with a payload, as it is in Python.** CPython builds an immutable builtin's value in `__new__`, from the *constructor's* arguments, before `__init__` is reached — so `class N(int)` whose `__init__` ignores them still answers `4` for `N(4.9)` — while `list.__new__` ignores them and hands `__init__` an empty list, which is why a mutable subclass with a signature of its own is an ordinary empty list carrying its own state. POOP wrote the payload in `__init__` alone, so a subclass that never passed it up came back with none at all and the first message answered `N does not understand #_value`, publishing the internal slot name. `_endow` supplies the missing step: one positional argument and no keywords is the shape every converter takes, so it converts; anything else is the subclass's own signature and the wrapper's *empty* value stands in; and where there is no empty to stand in (an `int` has none) the object cannot be built, which is what CPython answers there too. `bool` is refused as a base outright, again where CPython refuses it — POOP used to let the class be defined and fail at the first instance with `Can't instantiate abstract class Flag without an implementation for abstract methods '__bool__', '__str__', …`, a dozen dunders in one sentence from a program that spelled none.

**`super().__init__(...)` converts, like the call it mirrors.** The third and last home of the same gap. `no_dunder_attribute` carves out exactly one dunder call, `__init__`, *for* this spelling — so it is the constructor a program is invited to write — and it reached the wrapper's variadic `__init__`, the "build from these elements" meaning: `class S(list)` calling `super().__init__(xs)` answered a list holding one list, `class N(int)` an int holding `4.9`, and `class D(dict)` answered `dict.__init__() takes 1 positional argument but 2 were given`. `super().__init__(*xs)` was the spelling that worked, with nothing to say so. The alias carries an `__init__` of its own now, which converts and fills the payload: the MRO puts it exactly where `super()` looks (`S.__mro__` is `(S, <alias>, List, …)`), so it intercepts that call and nothing else. Over-supply answers what the direct call answers — `super().__init__(*xs)` now says `list is built from at most one collection, got 2 arguments`, which is CPython's own reply to `list.__init__(self, 1, 2)`. The three spellings of one intent — `list(xs)`, `Sub(xs)`, `super().__init__(xs)` — finally agree.
...
**A `MemoryView` prints what it is, not where it lives.** `__str__` was `repr(self._value)`, so `memoryview(b"ab").print()` answered `<memory at 0x70cb7ab59240>` — the raw pointer `Object.__hash__` refuses to answer for the reason its own comment gives, under the class name `memory`, which is neither the POOP name nor the cloak, and unstable across runs so that no test could pin it and no example could show it. It answers `<memoryview of 2 bytes>` instead. Printing the bytes themselves would re-materialize an arbitrarily large buffer just to print it — the cost a `Range` refuses when sliced — so the contents are shown on request by `hex()`, and `slice(...)` and `includes(x)` complete the set of messages the `no_subscript` and `no_in` bans point at: `mv[0:2]` is a `memoryview` in CPython and `98 in memoryview(b"ab")` is `True`, and a view you cannot look into is one gap with four spellings. `index(x)` and `count(x)` finish the sequence protocol beside them — every sibling answers both, and this was the one receiver a program could be told the element is there by and then not ask where.
...
- **The substitute was invisible.** `dir()` on an exception class did not list `raise_`, so `:methods` could not show what `no_raise` points at. Making it a message did not fix that on its own — `type.__dir__` never merged the metaclass's names either; `PoopMeta.__dir__` (above) is what finally lists it, for the exception classes that answer it and no others.
