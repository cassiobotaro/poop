...
Seventeen, not "~100+": Python 3.14 has 71 builtin exceptions, and a language with no files and no modules can never raise the `OSError` subtree. `RecursionError` is in the list because recursion is POOP's substitute for every loop, which makes it the most reachable of the lot.
