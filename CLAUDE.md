...
- `poop/transformers/` — AST transformers (rewrite nodes before execution); registered in `DEFAULT_TRANSFORMERS`: `boolean`, `none`, `complex`, `bytes`, `byte_array`, `memory_view`, `int`, `float`, `string`, `enumerate`, `zip`, `range`, `list`, `tuple`, `dict`, `set`, `frozen_set`, `raise_`, `class_`, `block`, `slice`. `try_`, `with_`, `path`, `math`, and `random` are namespace-only — they inject `Try`/`With`/`Path`/`Math`/`Random` into `DEFAULT_NAMESPACE` without rewriting AST. Every other type wrapper (`Int`, `List`, `Object`, ...) is bound under a mangled `_poop_*` name and unreachable from user code; lowercase Python builtins (`int`, `list`, `object`, ...) get rewritten to those mangled names.
