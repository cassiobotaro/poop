...
- `poop/validators/` — AST validators rejecting forbidden constructs (`if`, loops, `len`, `print`, 69 in all); source of truth: `DEFAULT_VALIDATORS` in `poop/validators/__init__.py`. `collect()` is the primitive: validators subclass `CollectingValidator` (`poop/validators/base.py`) and implement `collect()`; `validate()` derives from it by raising the first error. Collecting cannot be built from raising, and `--validators-only` needs every error.
