...
2. Define `BINDINGS: ClassVar[dict[str, object]]` on the transformer — these are injected into the runtime namespace so the rewritten AST can resolve the replacement names. Namespace-only modules (`try_`, `with_`) export a `NAMESPACE` dict instead and skip the rewriter.
